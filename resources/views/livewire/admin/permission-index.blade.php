<div>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 space-y-4 sm:space-y-0">
        <div>
            <h2 class="text-2xl font-extrabold text-slate-800 dark:text-slate-100 font-display">{{ __('manage.permissions.title') }}</h2>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">{{ __('manage.permissions.subtitle') }}</p>
        </div>
        <button wire:click="create" class="inline-flex items-center space-x-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-sm font-bold transition-all shadow-sm">
            ...
            <span>{{ __('manage.permissions.add_button') }}</span>
        </button>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm whitespace-nowrap">
                <thead class="bg-slate-50 dark:bg-slate-900/50 text-slate-600 dark:text-slate-400 border-b border-slate-200 dark:border-slate-700">
                    <tr>
                        <th class="px-6 py-4 font-bold">{{ __('manage.permissions.col_id') }}</th>
                        <th class="px-6 py-4 font-bold">{{ __('manage.permissions.col_name') }}</th>
                        <th class="px-6 py-4 font-bold text-right">{{ __('manage.permissions.col_actions') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50">
                    @forelse($permissions as $permission)
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                        <td class="px-6 py-4 font-medium text-slate-900 dark:text-slate-100">{{ $permission->id }}</td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400">
                                {{ $permission->name }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right space-x-2">
                            <button wire:click="edit({{ $permission->id }})" class="p-2 text-amber-500 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/30 rounded-lg transition-colors">
                                <i data-lucide="edit" class="w-4 h-4"></i>
                            </button>
                            <button wire:click="delete({{ $permission->id }})" wire:confirm="{{ __('manage.permissions.confirm_delete') }}" class="p-2 text-red-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors">
                    ...
                    <tr>
                        <td colspan="3" class="px-6 py-8 text-center text-slate-500 dark:text-slate-400">
                            {{ __('manage.permissions.not_found') }}
                        </td>
                    </tr>
                    @endforelse
                </tbody>
            ...
            <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700">
                ...
                <div class="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-700">
                    <h3 class="text-lg font-bold text-slate-900 dark:text-white">
                        {{ $permissionId ? __('manage.permissions.modal_title_edit') : __('manage.permissions.modal_title_add') }}
                    </h3>
                ...
                <form wire:submit.prevent="store" class="p-4 space-y-4">
                    <div>
                        <label class="block mb-2 text-sm font-bold text-slate-900 dark:text-white">{{ __('manage.permissions.field_name') }}</label>
                        <input type="text" wire:model="name" placeholder="{{ __('manage.permissions.field_name_placeholder') }}"
